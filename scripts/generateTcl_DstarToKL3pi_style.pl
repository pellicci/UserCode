...
foreach $tclfile (@LIST) {
  ...
  print OUTFILE "# === This file was generated automatically by generateTcl_DstarToKL3pi_style.pl of K0LUser package \n";
  ...
  $barefile =~ s/$rest//;
  $rootfile="$RootDir"."$barefile".".root";
  ...
  print OUTFILE "sourceFoundFile ../K0LUser/K0LRecoD0_KLPiPiPi0.tcl \n";
  close (OUTFILE);
  system ("\mv out.txt ccbarMCTcl/$barefile.tcl");
  system ("\rm out.txt");
}
...
sub help(){
  ...
  print "generateTcl_DstarToKL3pi_style.pl tcl1, tcl2, ... tcln \n";
  print "or:\n";
  print "generateTcl_DstarToKL3pi_style.pl -d tcldir/ \n";
