...
process.source = cms.Source("PoolSource",
   ...
   ##'/store/express/HIRun2010/HIExpressPhysics/FEVT/Express-v3/000/150/431/EC1B2E90-21EB-DF11-AD93-001D09F254CE.root'
   )
)                                                
...
process.primaryVertexFilter = cms.EDFilter("VertexSelector",
   ...
   cut = cms.string("!isFake && ndof > 4 && abs(z) <= 25 && position.Rho <= 2"), # tracksSize() > 3 for the older cut NEW for HI
...
### No scraping applied in HI, the track collection is hard code and the name changed, not sure also if it has sense for HI
### For the moment we skip this filter 
process.p = cms.Path(process.primaryVertexFilter* process.dtunpacker * process.dttfunpacker  + process.sources + process.MEtoEDMConverter  + process.DTOfflineAnalyzer )
