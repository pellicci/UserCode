...
Bool_t doChi2 = kFALSE;
  ...
  RooRealVar c1m("c1m","c1m",-0.055,-4.,4.);
  RooRealVar c2p("c2p","c2p",-4.,4.);
  RooRealVar c2m("c2m","c2m",0.452,-4.,4.);
  ...
  cout << "senza taglio " << dataWS_1->numEntries() << endl;
  cout << "con taglio " << dataWS->numEntries() << endl;
  ...
  RooTimepdf TOTsigD023("TOTsigD023","TOTsigD023",d0Lifetime,m2Kpi_d0mass,mass13a,gm,&dalitzSpace,tau,c1p,c2p,1,dirname);
  ...
  RooDataSet *ProtoData_flav = (RooDataSet*)RSdata->reduce(RooArgSet(D0flav));
  ...
  RooDataSet *ProtoData_flav_bkg = (RooDataSet*)bkgdata->reduce(RooArgSet(D0flav));
  ...
  if(doFit){
    RooNLLVar nll("nll","nll",TOTpdf,*dataWS);
    RooMinuit m(nll);
   ...
   if(doPlots){
     ...
     TOTpdf_plot.plotOn(tframe,Components(RooArgSet(*totmis_norm,totbkg_norm_plot)),DrawOption("F"),FillColor(kGreen));
     TOTpdf_plot.plotOn(tframe,Components(RooArgSet(totbkg_norm_plot)),DrawOption("F"),FillColor(kRed));
     tframe->getAttLine()->SetLineWidth(1);
     tframe->getAttLine()->SetLineStyle(1);
     tframe->SetTitle("");
     tframe->GetXaxis()->SetTitle("t_{D^{0}}");
     tframe->GetYaxis()->SetTitle("");
    ...
    xframe->SetTitle("");
    xframe->GetXaxis()->SetTitle("m^{2}_{K#pi}");
    Double_t chi2Kpi = xframe->chiSquare();
    TOTpdf.plotOn(xframe,Project(RooArgSet(m2Kpi0_d0mass,d0Lifetime,d0LifetimeErr)),ProjWData(*dshist),Components(RooArgSet(*totmis_norm,totbkg_norm)),DrawOption("F"),FillColor(kRed));
    TCanvas c1;
    c1.cd();xframe->Draw();
    ...
    yframe->SetTitle("");
    yframe->GetXaxis()->SetTitle("m^{2}_{K#pi^{0}}");
    Double_t chi2Kpi0 = yframe->chiSquare();
    TOTpdf.plotOn(yframe,Project(RooArgSet(m2Kpi_d0mass,d0Lifetime,d0LifetimeErr)),ProjWData(*dshist),Components(RooArgSet(*totmis_norm,totbkg_norm)),DrawOption("F"),FillColor(kRed));
    TCanvas c2;
    c2.cd();yframe->Draw();
    ...
    TOTpdf23.plotOn(zframe,Project(RooArgSet(m2Kpi_d0mass,d0Lifetime,d0LifetimeErr)),ProjWData(*dshist),Components(RooArgSet(totmis_norm,totbkg_norm)),DrawOption("F"),FillColor(kRed));
