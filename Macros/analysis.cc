...
void tria(int vubd, int dmdd, int epsd, int dmsd, int s2bd, int c2bd, 
	  ...
	  int btovgd, int D0p0d, int kl2p0nnd, int BtoKppd, char DrawOpt[]) {

  // Draw 2bpg
  if (s2bpgd == 1)
    setdrawlevel(s2bpgConstrain,s2bpglevel,s2bpgsel, DrawOpt);
  if (s2bpgd == 2) {
    if (DrawOpt=="AREA") {
      if (s2bpgsel[4] > 0)
	if (s2bpgsel[3] > 0) {
...
//	  drawFromGraph(s2bpghisto, 1, "AREA", s2bpgsel[3]);
  ...
  // Draw c2b
  if (c2bd == 1) 
    setdrawlevel(c2bConstrain,c2blevel,c2bsel,DrawOpt);
  if (c2bd == 2) {
    if (DrawOpt=="AREA") {
      if (c2bsel[4] > 0)
	if (c2bsel[3] > 0) {
  ...
  // Draw Gamma
  if (gammad == 1)
    setdrawlevel(gammaConstrain,gammalevel,gammasel, DrawOpt);
  if (gammad == 2) {
    if (DrawOpt=="AREA") {
      if(gammasel[4] > 0)
	if(gammasel[3] > 0) {
  ...
  // Draw 2a
  if (s2ad == 1)
    setdrawlevel(s2aConstrain,s2alevel,s2asel, DrawOpt);
  if (s2ad == 2) {
    if (DrawOpt=="AREA") {
      if (s2asel[4] > 0)
	if (s2asel[3] > 0) {
  ...
  // Draw kpnn
  if (k2pnnd == 1)
    setdrawlevel(k2pnnConstrain,k2pnnlevel,k2pnnsel, DrawOpt);
  if (k2pnnd == 2) {
    if (DrawOpt=="AREA") {
      if (k2pnnsel[4] > 0)
	if (k2pnnsel[3] > 0) {
	  drawfunc(rho2_k2pnn, eta2_k2pnn, k2pnnsel[4], DrawOpt);
	  drawfunc(rho1_k2pnn, eta1_k2pnn, k2pnnsel[3], DrawOpt);
	} else 
	  drawfunc(rho2_k2pnn, eta2_k2pnn, k2pnnsel[4], DrawOpt);
	...
	drawfunc(rho1_k2pnn, eta1_k2pnn, k2pnnsel[3], DrawOpt);
      ...
      if (k2pnnsel[0] > 0)
	drawfunc(rho1_k2pnn, eta1_k2pnn, k2pnnsel[0], DrawOpt);
      if (k2pnnsel[1] > 0)
 	drawfunc(rho2_k2pnn, eta2_k2pnn, k2pnnsel[1], DrawOpt);
    }
  }

  // Draw klp0nn
  if (kl2p0nnd == 1)
    setdrawlevel(kl2p0nnConstrain,kl2p0nnlevel,kl2p0nnsel, DrawOpt);
  if (kl2p0nnd == 2) {
    if (DrawOpt=="AREA") {
      if (kl2p0nnsel[4] > 0)
	if (kl2p0nnsel[3] > 0) {
  ...
  // Draw BtoKpp
  if (BtoKppd == 1) 
    setdrawlevel(BtoKppConstrain,BtoKpplevel,BtoKppsel,DrawOpt);
  if (BtoKppd == 2) {
    if (DrawOpt=="AREA") {
      if (BtoKppsel[4] > 0)
	if (BtoKppsel[3] > 0) {
  ...
  // Draw btovg
  if (btovgd == 1) 
    setdrawlevel(btovgConstrain,btovglevel,btovgsel,DrawOpt);
  if (btovgd == 2) {
    if (DrawOpt=="AREA") {
      if (btovgsel[4] > 0)
	if (btovgsel[3] > 0) {
	  drawfunc(rho2_btovg, eta2_btovg, btovgsel[4], DrawOpt);
	  drawfunc(rho1_btovg, eta1_btovg, btovgsel[3], DrawOpt);
	} else
	  drawfunc(rho2_btovg, eta2_btovg, btovgsel[4], DrawOpt);
	...
	drawfunc(rho1_btovg, eta1_btovg, btovgsel[3], DrawOpt);
      ...
      if (btovgsel[0] > 0)
 	drawfunc(rho1_btovg, eta1_btovg, btovgsel[0], DrawOpt);
      if (btovgsel[1] > 0)
 	drawfunc(rho2_btovg, eta2_btovg, btovgsel[1], DrawOpt);
    }
  }

  // Draw dmd
  if (dmdd == 1) 
    setdrawlevel(dmdConstrain,dmdlevel,dmdsel,DrawOpt);
  if (dmdd == 2) {
    if (DrawOpt=="AREA") {
      if (dmdsel[4] > 0)
	if (dmdsel[3] > 0) {
	  drawfunc(rho2_dmd, eta2_dmd, dmdsel[4], DrawOpt);
	  drawfunc(rho1_dmd, eta1_dmd, dmdsel[3], DrawOpt);
	} else
	  drawfunc(rho2_dmd, eta2_dmd, dmdsel[4], DrawOpt);
      else if (dmdsel[3] > 0)
	drawfunc(rho1_dmd, eta1_dmd, dmdsel[3], DrawOpt);
      ...
      if (dmdsel[0] > 0)
 	drawfunc(rho1_dmd, eta1_dmd, dmdsel[0], DrawOpt);
      if (dmdsel[1] > 0)
 	drawfunc(rho2_dmd, eta2_dmd, dmdsel[1], DrawOpt);
    }
  }
  ...
  if (dmsd == 1) { 
    ...
    setdrawlevel(dmsConstrain,dmslevel,dmssel,DrawOpt);
  }
  if ( dmsd==2 ) {
    if(doMFV != 0) {
      if (DrawOpt=="AREA") {
	if (dmssel[4] > 0)
	  if (dmssel[3] > 0) {
	    drawfunc(rho2_dms, eta2_dms, dmssel[4], DrawOpt);
	    drawfunc(rho1_dms, eta1_dms, dmssel[3], DrawOpt);
	  } else
	    drawfunc(rho2_dms, eta2_dms, dmssel[4], DrawOpt);
	else if (dmssel[3] > 0)
	  drawfunc(rho1_dms, eta1_dms, dmssel[3], DrawOpt);
	...
	if (dmssel[0] > 0)
	  drawfunc(rho1_dms, eta1_dms, dmssel[0], DrawOpt);
	if (dmssel[1] > 0)
	  drawfunc(rho2_dms, eta2_dms, dmssel[1], DrawOpt);
      }
    } else {
      if (DrawOpt=="CONT") {
	if (dmssel[1] > 0)
	  drawfunc(rho2_dms, eta2_dms, dmssel[1], "DASHED");
      } else if (DrawOpt=="AREA") {
	if (dmssel[4] > 0)
	  drawfunc(rho2_dms, eta2_dms, dmssel[4], DrawOpt);
      }
    }
  ...
  // Draw b -> tau nu
  if (b2taunud == 1) 
    setdrawlevel(b2taunuConstrain,b2taunulevel,b2taunusel,DrawOpt);
  if (b2taunud == 2) {
    if (DrawOpt=="AREA") {
      if (b2taunusel[4] > 0)
	if (b2taunusel[3] > 0) {
	  drawfunc(rho2_b2taunu, eta2_b2taunu, b2taunusel[4], DrawOpt);
	  drawfunc(rho1_b2taunu, eta1_b2taunu, b2taunusel[3], DrawOpt);
	} else
	  drawfunc(rho2_b2taunu, eta2_b2taunu, b2taunusel[4], DrawOpt);
 	...
 	drawfunc(rho1_b2taunu, eta1_b2taunu, b2taunusel[3], DrawOpt);
      ...
      if (b2taunusel[0] > 0)
 	drawfunc(rho1_b2taunu, eta1_b2taunu, b2taunusel[0], DrawOpt);
      if (b2taunusel[1] > 0)
 	drawfunc(rho2_b2taunu, eta2_b2taunu, b2taunusel[1], DrawOpt);
    }
  }

  // Draw Vub
  if (vubd == 1) 
    setdrawlevel(vubConstrain,vublevel,vubsel,DrawOpt);
  if (vubd == 2) {
    if (DrawOpt=="AREA") {
      if (vubsel[4] > 0)
	if (vubsel[3] > 0) {
	  drawfunc(rho2_vub, eta2_vub, vubsel[4], DrawOpt);
	  drawfunc(rho1_vub, eta1_vub, vubsel[3], DrawOpt);
	} else
	  drawfunc(rho2_vub, eta2_vub, vubsel[4], DrawOpt);
      else if (vubsel[3] > 0)
 	drawfunc(rho1_vub, eta1_vub, vubsel[3], DrawOpt);
      ...
      if (vubsel[0] > 0)
 	drawfunc(rho1_vub, eta1_vub, vubsel[0], DrawOpt);
      if (vubsel[1] > 0)
 	drawfunc(rho2_vub, eta2_vub, vubsel[1], DrawOpt);
    }
  }

  // Draw epsk
  if (epsd == 1) 
    setdrawlevel(epsConstrain,epslevel,epssel,DrawOpt);
  if ( epsd==2 ) {
    if (DrawOpt=="AREA") {
      if (epssel[4] > 0) {
	if (epssel[3] > 0) {
	  drawfunc200(rho2_eps, eta2_eps, epssel[4], DrawOpt);
	  drawfunc200(rho1_eps, eta1_eps, epssel[3], DrawOpt);
	  drawfunc200(rho2m_eps, eta2m_eps, epssel[4], DrawOpt);
	  drawfunc200(rho1m_eps, eta1m_eps, epssel[3], DrawOpt);
	} else {
	  drawfunc200(rho2_eps, eta2_eps, epssel[4], DrawOpt);
	  drawfunc200(rho2m_eps, eta2m_eps, epssel[4], DrawOpt);
	}
	...
	drawfunc200(rho1m_eps, eta1m_eps, epssel[3], DrawOpt);
      ...
      if (epssel[0] > 0) {
	drawfunc200(rho1_eps, eta1_eps, epssel[0], DrawOpt);
	drawfunc200(rho1m_eps, eta1m_eps, epssel[0], DrawOpt);
      }
      if (epssel[1] > 0) {
	drawfunc200(rho2_eps, eta2_eps, epssel[1], DrawOpt);
	drawfunc200(rho2m_eps, eta2m_eps, epssel[1], DrawOpt);
      }
    }
  }
  ...
  // Draw s2b
  if (s2bd == 1) 
    setdrawlevel(s2bConstrain,s2blevel,s2bsel,DrawOpt);
  if (s2bd == 2) {
    if (DrawOpt=="AREA") {
      if (s2bsel[4] > 0)
	if (s2bsel[3] > 0) {
...
void cos2b_areas(double twob_min, double twob_max, int color, char side[]) {    
...
//   B2taunu
  if(set_b2taunu[0] == 2) {
    TH1D* tmp_b2taunu = (TH1D*) datafile->Get("Input/input_btaunu")->Clone();
...
//  epsilon  
  ...
  if(set_gamma[0] == 2) {
    ...
    TH1D* tmp_gamma = (TH1D*) datafile->Get(gamma_tit)->Clone();
    //    TFile pippo("winter07.root");
    //    TH1D* tmp_gamma = (TH1D*) pippo.Get(gamma_tit)->Clone();
  ...
  if(set_s2bpg[0] == 2) {
    ...
    if(s2bpg_use_his == 1) {
      s2bpg1D->Multiply((TH1D*) datafile->Get("Input/input_2bpg_star"));
      s2bpg1D->Multiply((TH1D*) datafile->Get("Input/input_2bpg_rho"));
    }
  ...
  // 2alpha
  if(set_s2a[0] == 2) {
    s2a1D = (TH1D*) datafile->Get("Input/pipi_input_alpha")->Clone();
    s2a1D->Multiply((TH1D*) datafile->Get("Input/rhorho_input_alpha"));
    s2a1D->Multiply((TH1D*) datafile->Get("Input/alpharhopi"));
...
void drawfunc(double rho[400], double eta[400], int nsind, char DrawOpt[]) {
  if (DrawOpt=="CONT") {
...
void drawfunc200(double rho[400], double eta[400], int nsind, char DrawOpt[]) {
  if (DrawOpt=="CONT") {
...
void setdrawlevel(TH2D* Constrain, double level[5], int sel[6], char DrawOpt[]) {
  if ( DrawOpt == "AREA" ) {
    if (sel[5] > 0) {
      if (sel[3] > 0 && sel[4] > 0) {
...
void drawcont(TH2D* ThisConstrain, double level[5], int nlevel, int color, char DrawOpt[]) {
  // set the level
  // read Plot infos
  ...
  if(DrawOpt == "AREA") {
	...
	if(ThisConstrain2->GetCellContent(ix,iy) <= level[4-nlevel]) {
	  ThisConstrain2->SetCellContent(ix,iy,0.);
	} else {
	  ThisConstrain2->SetCellContent(ix,iy,pal);
	}
       }
    }
...
double CalcLevel(TH1D* InputPlot, double conf_level, char* side) {
